...
var cotarId = '5239d234aaa5cd604208b24a';
var fungId = '5239daf4aaa5cd604208b2ad';
var changId = '5239b87baaa5cd1c65cd50fb';
var bodnarId = '523a1275aaa5cd604208b425';
var harrisonId = '523b14d8aaa5cd604208b8e3'
...
bot.on('speak', function (data) {
  ...
  } else if (text.match(/^\/catfact$/i)){
      tellCatFact();
  } else if (text.match(/^\/hello$/i)) {
      bot.speak('Hey! How are you @'+name+'?');
  } else if (text.match(/^\/goodbye$/i)) {
      bot.speak('See ya later, @'+name+'!');
...
bot.on('add_dj', function (data) {
  bot.speak("Watch out! " + data.user[0].name + " is taking the stage!");   
});
...
var tellCatFact = function(){
  var catSize = catFacts.length;
  var random = Math.floor((Math.random()*catSize));
  bot.speak(catFacts[random]);
}


var catFacts = ["Cats have five toes on each front paw, but only four toes on each back paw.",
